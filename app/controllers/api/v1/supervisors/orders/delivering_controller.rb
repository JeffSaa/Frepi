class  Api::V1::Supervisors::Orders::DeliveringController < Api::V1::ApiController
  skip_before_action :authenticate_user!, :require_administrator

  def index
    if params[:page]
      @orders = Order.where(active: true, status: 2).order(:scheduled_date, :expiry_time).paginate(per_page: params[:per_page], page: params[:page])
